...
ConfigBase.prototype.get = function (name) {
  ...
  if (typeof value === 'undefined') {
    throw new Error('config field `' + name + '` in ' + this._project + '.' + this._version + ' is undefined');
  }
  return value;
};
...
ConfigBase.prototype.versions = function (project) {
  return this._superConfig.versions(project || this._project);
};

ConfigBase.prototype.versionExists = function (version, project) {
  return (this.versions(project).indexOf(version) !== -1);
};
